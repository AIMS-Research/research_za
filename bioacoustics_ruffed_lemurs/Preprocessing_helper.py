...
class Preprocessing:
    ...
    def __init__(self, species_folder, lowpass_cutoff, 
                 ...
                 augmentation_amount_background_class,file_name,
        ...
        self.training_files = self.species_folder + '/DataFiles/'+file_name
        self.n_ftt = n_fft
    ...
    def getXY(self, audio_amplitudes, start_sec, annotation_duration_seconds, label, verbose):
        ...
        if label in self.background_class:
            if segments_to_extract > 4:
                segments_to_extract = 4
            
        for i in range (0, segments_to_extract):
            if verbose:
                print ('Segment {} of {}'.format(i, segments_to_extract-1))
                print ('*******************')
    ...
    def save_data_to_pickle(self, X, Y, X_name, Y_name):
        '''
        Save all of the spectrograms to a pickle file.
        
        '''
        outfile = open(os.path.join(self.saved_data_path, X_name+'-pow.pkl'),'wb')
        pickle.dump(X, outfile, protocol=4)
        outfile.close()
        
        outfile = open(os.path.join(self.saved_data_path, Y_name+'-pow.pkl'),'wb')
        pickle.dump(Y, outfile, protocol=4)
        outfile.close()
        
    def load_data_from_pickle(self, X_name, Y_name):
        '''
        Load all of the spectrograms from a pickle file
        
        '''
        infile = open(os.path.join(self.saved_data_path, X_name+'-pow.pkl'),'rb')
        X = pickle.load(infile)
        infile.close()
        
        infile = open(os.path.join(self.saved_data_path, Y_name+'-pow.pkl'),'rb')
    ...
    def create_dataset(self, verbose):
        ...
        for training_file in training_files.values:
            ...
            print('new location', reader.get_audio_location())
            ...
            if file_name_no_extension+self.audio_extension  in listdir(self.audio_path):
                ...
                df, af = reader.get_annotation_information()
